...
export function formatAjvError(err: ErrorObject, form = "ITR-4"): OfficialSchemaError {
  ...
  if (keyword === "required") explanation = `Required official ${form} field is missing: ${err.params?.missingProperty || field}.`;
  ...
  if (keyword === "type") explanation = `${field} has the wrong type for the official ${form} schema.`;
